...
export function LoginButton({
  ...
  // Default styles per provider (can be overridden via props)
  const defaultVariant = provider === 'github' ? 'solid' : 'outline';
  const defaultColor = provider === 'github' ? 'gray' : undefined;
  const defaultHighContrast = provider === 'github';
  ...
  return (
    ...
    >
      {isLoading ? <Spinner size="2" /> : config.icon}
